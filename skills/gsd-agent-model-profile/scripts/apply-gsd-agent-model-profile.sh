...
def render_profile(text: str, effort: str) -> str:
    ...
    prefix = f'model = "{TARGET_MODEL}"\nmodel_reasoning_effort = "{effort}"\n'
    return prefix + "".join(output)


def verify_text(text: str, effort: str) -> tuple[bool, str]:
    fields = top_level_fields(text)
    expected = {"model": TARGET_MODEL, "model_reasoning_effort": effort}
    problems: list[str] = []
...
def main(argv: list[str]) -> int:
    ...
    for agent_name, effort in PROFILES.items():
        ...
        if args.verify:
            ok, reason = verify_text(original, effort)
            if ok:
                print(f"OK {path}")
            else:
        ...
        desired = render_profile(original, effort)
        if args.dry_run:
            if desired == original:
                print(f"OK {path} model={TARGET_MODEL} model_reasoning_effort={effort}")
            else:
                print(f"WOULD_UPDATE {path} model={TARGET_MODEL} model_reasoning_effort={effort}")
            continue
        ...
        changed += 1
        print(f"UPDATED {path} model={TARGET_MODEL} model_reasoning_effort={effort}")
