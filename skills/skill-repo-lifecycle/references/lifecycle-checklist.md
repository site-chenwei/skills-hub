...
- Enumerate only directories under `skills/` that contain `SKILL.md`.
- Each repo-owned skill should have `SKILL.md` and `agents/openai.yaml`.
- Add `run.py` and tests when the skill has deterministic helper behavior.
- Keep detailed workflows in `references/` when `SKILL.md` would become too large.
...
- Move domain workflows into skills: DocsHub lookup, Harmony build, structured development, verification, code review, Git delivery, and skill repository lifecycle.
...
- Sync only repo-owned skill folders.
...
- Treat a repo-owned source skill that is missing under the install root as an attention item, not as implicit success.
