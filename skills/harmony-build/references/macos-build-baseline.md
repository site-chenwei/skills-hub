...
- `build` 是面向 agent 的高层构建入口；未显式传 `--task` 时固定运行裸 `assembleHap`，不列公开 tasks 选择任务，也不推断 `:<module>:assembleHap`。非 JSON 模式会输出阶段进度，最终输出成功/失败、阶段、任务、退出码和耗时。成功时不回传 hvigor 日志，失败时才回传错误尾部。
- `build --timeout-seconds` 是整条 build flow 的 hvigor 等待预算，默认 900 秒。超时后进程清理会有短暂兜底等待。
